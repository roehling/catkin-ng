...
#
##############################################################################
set(HAZEL_PACKAGE_EXPORTS)
set(HAZEL_PACKAGE_EXPORTED_DEPENDS)
set(HAZEL_PACKAGE_CATKIN_DEPENDS)
set(HAZEL_PACKAGE_EXPORTED_CMAKE_FILES)
...
function(hazel_export)
    ...
    if(arg_EXPORT)
        if(arg_EXPORT STREQUAL "${PROJECT_NAME}")
            message(FATAL_ERROR "export set must have a different name than the package. Use, e.g., '${PROJECT_NAME}Targets' instead.")
        endif()
        if(NOT arg_FILE)
        ...
        foreach(target IN LISTS arg_TARGETS)
            ...
            foreach(pkg IN LISTS HAZEL_IMPORTED_PACKAGES)
                foreach(dep IN LISTS depends)
                    if(dep IN_LIST ${pkg}_EXPORTED_TARGETS)
                        message(STATUS "hazel_export: exported target '${target}' depends on package '${pkg}'")
                        list(APPEND HAZEL_PACKAGE_EXPORTED_DEPENDS "${pkg}")
            ...
            endforeach()
            foreach(dep IN LISTS depends)
                if(dep MATCHES "^(pkg-config|catkin)::")
                    message(STATUS "hazel_export: exported target '${target}' depends on imported target '${dep}'")
                    list(APPEND HAZEL_PACKAGE_EXPORTED_DEPENDS "${dep}")
                endif()
            endforeach()
        endforeach()
    endif()
    if(arg_EXPORT AND NOT arg_EXPORT IN_LIST HAZEL_PACKAGE_EXPORTS)
        ...
        list(APPEND HAZEL_PACKAGE_EXPORTS "${arg_EXPORT}")
        list(APPEND HAZEL_PACKAGE_EXPORTED_CMAKE_FILES "${arg_FILE}")
        list(APPEND HAZEL_PACKAGE_EXPORTED_TARGET_FILES "${arg_FILE}")
    endif()
    if(arg_CMAKE_SCRIPTS)
        _hazel_export_cmake_scripts(${arg_CMAKE_SCRIPTS})
    endif()
    set(HAZEL_PACKAGE_EXPORTS "${HAZEL_PACKAGE_EXPORTS}" PARENT_SCOPE)
    set(HAZEL_PACKAGE_EXPORTED_CMAKE_FILES "${HAZEL_PACKAGE_EXPORTED_CMAKE_FILES}" PARENT_SCOPE)
    set(HAZEL_PACKAGE_EXPORTED_TARGET_FILES "${HAZEL_PACKAGE_EXPORTED_TARGET_FILES}" PARENT_SCOPE)
    set(HAZEL_PACKAGE_EXPORTED_DEPENDS "${HAZEL_PACKAGE_EXPORTED_DEPENDS}" PARENT_SCOPE)
endfunction()
...
macro(_hazel_export_cmake_scripts)
    ...
    foreach(script IN ITEMS ${ARGN})
        ...
        list(APPEND HAZEL_PACKAGE_EXPORTED_CMAKE_FILES "${script_name}")
    endforeach()
endmacro()
