...
function(hazel_python_setup)
    ...
    if(EXISTS "${arg_SRCDIR}/setup.py")
        if(HAZEL_DEVEL_PREFIX)
            ...
            file(MAKE_DIRECTORY "${HAZEL_DEVEL_PYTHON_DIR}" "${CMAKE_CURRENT_BINARY_DIR}/python-setup/build")
            set(HAZEL_GENERATED_DIR "${CMAKE_CURRENT_BINARY_DIR}/hazel-generated")
            file(WRITE "${HAZEL_GENERATED_DIR}/${PROJECT_NAME}.pth")
            add_custom_command(OUTPUT "${HAZEL_GENERATED_DIR}/python-develspace.stamp"
                ...
                COMMAND "${CMAKE_COMMAND}" -E env "PYTHONPATH=${HAZEL_DEVEL_PYTHON_DIR}" "${HAZEL_PYTHON_EXECUTABLE}" -m pip install --no-deps -b "${CMAKE_CURRENT_BINARY_DIR}/python-setup/build" --prefix "${HAZEL_DEVEL_PREFIX}" "--install-option=--install-dir=${HAZEL_DEVEL_PYTHON_DIR}" "--install-option=--script-dir=${HAZEL_DEVEL_PREFIX}/${script_bindir}" --editable "${arg_SRCDIR}"
        ...
        install(CODE "execute_process(WORKING_DIRECTORY \"${arg_SRCDIR}\" COMMAND ${HAZEL_PYTHON_EXECUTABLE} setup.py build -b \"${CMAKE_CURRENT_BINARY_DIR}/python-setup/build\" egg_info -e \"${CMAKE_CURRENT_BINARY_DIR}/python-setup/egg-info\" install \"--install-scripts=${CMAKE_INSTALL_PREFIX}/${script_bindir}\" \"--install-lib=${CMAKE_INSTALL_PREFIX}/${HAZEL_PYTHON_INSTALL_PACKAGES_DIR}\" --root \"\$ENV{DESTDIR}/\" --prefix \"${CMAKE_INSTALL_PREFIX}\")")
    else()
