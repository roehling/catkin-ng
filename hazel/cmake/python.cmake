    ...
    if(EXISTS "${arg_DIRECTORY}/setup.py")
        if(HAZEL_DEVEL_PREFIX)
            ...
            add_custom_command(OUTPUT "${HAZEL_GENERATED_DIR}/python-develspace.stamp"
                ...
                COMMAND "${CMAKE_COMMAND}" -E env "PYTHONPATH=${PYTHONPATH}" "${HAZEL_PYTHON_EXECUTABLE}" -m pip install --no-deps -b "${CMAKE_CURRENT_BINARY_DIR}/python-setup/build" --prefix "${HAZEL_DEVEL_PREFIX}" "--install-option=--install-dir=${HAZEL_DEVEL_PYTHON_DIR}" "--install-option=--script-dir=${HAZEL_DEVEL_PREFIX}/${script_bindir}" --editable "${arg_DIRECTORY}"
