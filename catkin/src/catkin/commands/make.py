...
def build_package(path, args):
    ...
    if not os.path.isfile(os.path.join(pkgbuilddir, build_script)):
        ...
        add_default_arg(cmake_args, f"-DCATKIN_DEVEL_PREFIX={develdir}")
...
def run(args):
    ...
    if not os.path.isdir(args.directory):
        sys.stderr.write(f"catkin_make: no such directory: {args.directory!r}\n")
        return 1
    srcdir = os.path.join(args.directory, args.source)
    if not os.path.isdir(srcdir):
        sys.stderr.write(f"catkin_make: no such directory: {srcdir!r}\n")
        return 1
        ...
        for name in tier:
            ...
            if failed_packages & dependency_graph[name]:
                ...
                try:
                    ...
                    sys.stderr.write(f"catkin_make: process exited with return code {e.returncode}\n")
...
def main():
    parser = argparse.ArgumentParser(description="build packages in catkin workspace")
    prepare_args(parser)
