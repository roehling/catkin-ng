...
def main():
    main_parser = argparse.ArgumentParser(prog=f"python{sys.version_info.major} -m catkin", description="catkin build system tool")
    sub_parsers = main_parser.add_subparsers(dest="command", metavar="ACTION")
    sub_parsers.required = True
    for command, info in COMMANDS.items():
        module = importlib.import_module(info.get("module", f".{command}"), "catkin.commands")
    ...
    module = importlib.import_module(info.get("module", f".{args.command}"), "catkin.commands")
