...
set(CATKIN_PACKAGE_EXPORTED_DEPENDS)
set(CATKIN_PACKAGE_EXPORTED_CMAKE_FILES)
set(CATKIN_PACKAGE_EXPORTED_TARGET_FILES)
...
function(catkin_export)
    ...
    if(arg_FILE)
        ...
        set(number 1)
        while(arg_FILE IN_LIST CATKIN_PACKAGE_EXPORTED_CMAKE_FILES)
            math(EXPR number "${number} + 1")
    ...
    if(arg_TARGETS)
        ...
        foreach(target IN LISTS arg_TARGETS)
            ...
            foreach(pkg IN LISTS CATKIN_IMPORTED_PACKAGES)
                foreach(dep IN LISTS depends)
                    if(dep IN_LIST ${pkg}_EXPORTED_TARGETS)
                        message(STATUS "catkin_export: exported target '${target}' depends on package '${pkg}'")
                        list(APPEND CATKIN_PACKAGE_EXPORTED_DEPENDS "${pkg}")
                        break()
                    endif()
                endforeach()
            endforeach()
    ...
    if(arg_EXPORT)
        ...
        list(APPEND CATKIN_PACKAGE_EXPORTED_CMAKE_FILES "${arg_FILE}")
        list(APPEND CATKIN_PACKAGE_EXPORTED_TARGET_FILES "${arg_FILE}")
    endif()
    if(arg_CMAKE_SCRIPTS)
        _catkin_export_cmake_scripts(${arg_CMAKE_SCRIPTS})
    endif()
    set(CATKIN_PACKAGE_EXPORTS "${CATKIN_PACKAGE_EXPORTS}" PARENT_SCOPE)
    set(CATKIN_PACKAGE_EXPORTED_CMAKE_FILES "${CATKIN_PACKAGE_EXPORTED_CMAKE_FILES}" PARENT_SCOPE)
    set(CATKIN_PACKAGE_EXPORTED_TARGET_FILES "${CATKIN_PACKAGE_EXPORTED_TARGET_FILES}" PARENT_SCOPE)
    set(CATKIN_PACKAGE_EXPORTED_DEPENDS "${CATKIN_PACKAGE_EXPORTED_DEPENDS}" PARENT_SCOPE)
endfunction()

macro(_catkin_export_cmake_scripts)
    ...
    foreach(script IN ITEMS ${ARGN})
        ...
        if(script_name IN_LIST CATKIN_PACKAGE_EXPORTED_CMAKE_FILES)
        ...
        list(APPEND CATKIN_PACKAGE_EXPORTED_CMAKE_FILES "${script_name}")
    endforeach()
endmacro()
